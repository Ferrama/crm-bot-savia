...
const MessagesList = ({ ticket, ticketId, isGroup, markAsRead }) => {
          ...
          <>
            ...
            >
              {data?.message?.imageMessage?.caption && (
                <>
                  <WhatsMarked>{data.message.imageMessage.caption}</WhatsMarked>
                </>
              )}
            </div>
          </>
    ...
    if (!document && message.mediaType === 'audio') {
      return (
        <>
          ...
          {message.body && !message.body.startsWith('Áudio') && (
  ...
  const renderMessageAck = (message) => {
    ...
    if (message.ack === 4) {
      return <CheckCheck size={16} className={classes.ackDoneReadIcon} />;
    }
  };
      ...
      return (
        <div>
          ...
          >
            ...
            >
              {i18n.t('internalChat.conversation')}
            </Typography>
          </div>
        </div>
      );
  ...
  const messageLocation = (message, createdAt) => {
    return (
      <div
        className={[classes.textContentItem, { display: 'flex', padding: 5 }]}
      >
        ...
        >
          {' '}
          {i18n.t('internalChat.clickToViewLocation')}
        </a>
            ...
            >
              ...
              {dataContext?.isForwarded && (
                <span className={classes.forwardedMessage}>
                  <Forward size={16} className={classes.forwardedIcon} />{' '}
              ...
              {message.mediaUrl &&
                ...
                checkMessageMedia(message, data)}
